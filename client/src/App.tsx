...
export default function App() {
  ...
  useEffect(() => {
    const change = () => setProfile(location.hash === "#profile");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api<{ user: User }>("/auth/me", { signal: controller.signal }).catch(
        (e: unknown) => {
    ...
    <main className="app-shell">
      <header className="site-header">
        <a className="brand" href="/">
          PICKS<span> CLUB.</span>
        </a>
        {user && (
          <div className="account-nav">
            {user.photoUrl && (
              <img className="member-avatar" src={user.photoUrl} alt="" />
            )}
            <span>{user.name}</span>
      ...
      {error && (
        <p role="alert" className="error-banner">
          {error}{" "}
          {!ready && <button onClick={() => location.reload()}>Retry</button>}
        </p>
      )}
      {!ready && !error && <p role="status">Opening the club…</p>}
      {emailLink.has("reset") || emailLink.has("verify") ? (
        <AccountHelp
          mode={emailLink.has("reset") ? "reset" : "verify"}
          token={emailLink.get("reset") ?? emailLink.get("verify") ?? ""}
        />
      ) : (
        ready &&
        config &&
        (user ? (
          !user.ageConfirmed ? (
            <AgeConsent onConfirmed={setUser} />
          ) : config.requireVerified && !user.emailVerified ? (
            <Verification />
          ) : profile ? (
      ...
      <footer className="app-footer">
        Picks Club · Made for your Saturday circle.
      </footer>
    </main>
  );
}
