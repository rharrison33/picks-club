...
async function feed<T>(
  ...
  if (existing && existing.expires > Date.now())
    return existing.data as Promise<T[]>;
  const data = (async () => {
    ...
    if (!response.ok)
      throw new Error("Sports data is temporarily unavailable.");
...
export function isSaturday(value: unknown): value is string {
  if (typeof value !== "string" || !/^20[2-9][0-9]-\d{2}-\d{2}$/.test(value))
    return false;
...
export async function getSlate(
  ...
  const calendar = await feed<CalendarWeek>(
    "calendar?year=" + year,
    86400_000,
    true,
  );
  const target = Date.parse(saturday + "T12:00:00Z");
  const week = calendar.find(
    (w) => target >= Date.parse(w.startDate) && target <= Date.parse(w.endDate),
  );
  if (!week)
    throw new Error("No college football week is available for this Saturday.");
  ...
  const [rawGames, pollWeeks, teams, records, lines] = await Promise.all([
    ...
    feed<PollWeek>(
      "rankings?year=" + year + "&seasonType=" + week.seasonType,
      3600_000,
    ),
  ...
  const latest = pollWeeks
    .filter(
      (w) =>
        (w.week ?? 0) <= week.week &&
        w.polls.some((p) => p.poll === "AP Top 25" && p.ranks.length),
    )
  ...
  const games = rawGames.filter(
    (g) =>
      !g.startTimeTBD &&
      Number.isFinite(Date.parse(g.startDate)) &&
      dateFormatter.format(new Date(g.startDate)) === saturday,
  );
  const lineMap = new Map(lines.map((g) => [g.id, g]));
  const enriched = enrichGames(games, rankings, teams, records).map((g) => ({
    ...g,
    odds: chooseOdds(lineMap.get(g.id)),
  }));
  ...
  suggestions.suggestedIds = suggestGames(
    enriched.filter((g) => Date.parse(g.startDate) > Date.now()),
    playerFavorites,
  ).suggestedIds;
  ...
  return {
    games: enriched.map((g) => ({
      ...g,
      recommendation: priorities.get(g.id)!,
    })),
    ...suggestions,
    saturday,
    timezone,
    season: year,
    week: week.week,
    rankingWeek: latest?.week ?? null,
    rankingsAvailable: !!latest,
    recordsAvailable: records.length > 0,
    oddsAvailable: enriched.some((g) => g.odds !== null),
  };
}
export type Slate = Awaited<ReturnType<typeof getSlate>>;
export async function getTeamOptions() {
  const teams = await feed<Team>(
    "teams?year=" + new Date().getUTCFullYear(),
    86400_000,
    true,
  );
  return [
