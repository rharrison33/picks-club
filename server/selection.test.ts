...
test("lineup deadline is Friday midnight in the pool time zone", () => {
  ...
  assert.equal(
    lineupOpen(
      "2026-09-19",
      "America/Denver",
      Date.parse("2026-09-18T05:59:59Z"),
    ),
    true,
  );
  assert.equal(
  ...
  assert.equal(
    lineupOpen(
      "2026-12-19",
      "America/Denver",
      Date.parse("2026-12-18T06:59:59Z"),
    ),
    true,
  );
  assert.equal(
    lineupOpen(
      "2026-12-19",
      "America/Denver",
      Date.parse("2026-12-18T07:00:00Z"),
    ),
    false,
  );
});
...
test("ranked close games precede even the most favorite-team games", () => {
  const games = [
    ...Array.from({ length: 12 }, (_, i) => game(i + 1)),
    game(20, "FL", 25, -7),
  ];
  assert.equal(
    suggestGames(games, [["Home 1"], ["Home 1"]]).suggestedIds[0],
    20,
  );
...
test("pool membership weights favorite-team preferences independently", () => {
  ...
  assert.equal(
    suggestGames(games, [["Home 1"], ["Home 1"], ["Home 2"]]).suggestedIds[0],
    1,
  );
  assert.equal(
    suggestGames(games, [["Home 2"], ["Home 2"], ["Home 1"]]).suggestedIds[0],
    2,
  );
  assert.deepEqual(
    suggestGames(games, [["Home 1"]]),
    suggestGames([...games].reverse(), [["Home 1"]]),
  );
