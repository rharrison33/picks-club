...
export function suggestGames(
  ...
  const priorities = games
    .map((game) => {
      ...
      const margin =
        spread === null || spread === undefined ? null : Math.abs(spread);
      ...
      const reasons = [
        ...
        ...(bothRanked
          ...
          : ranked
            ? ["AP-ranked team"]
            : []),
        ...(margin !== null && margin <= 7 && !mustWatch
          ? ["Close spread"]
          : []),
      ];
      return {
    ...
    .sort(
      (a, b) =>
        Number(b.mustWatch) - Number(a.mustWatch) ||
        b.score - a.score ||
  ...
  return {
    ...
    priorities,
    selectionNote:
      "Ranked teams in games with a spread of 7 or less come first. Members' favorite teams, closer spreads, and AP rank decide the rest.",
  };
}
