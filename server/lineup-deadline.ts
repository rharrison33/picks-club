...
export function lineupOpen(saturday: string, timezone: string, now = Date.now()) {
  ...
  return local >= monday.toISOString().slice(0, 10) + "T11:00" && local < friday.toISOString().slice(0, 10) + "T00:00";
}
