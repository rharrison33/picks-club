...
test("pick deadlines, private choices, scoring corrections, and single-use recovery", async () => {
  ...
  for (const id of ["alice", "bob"]) {
    await db
      ...
      .run(id, id + "@example.test", id, "CO", "unused");
    await db
  ...
  await db
    .prepare(
      "INSERT INTO pools(id,name,invite_code,timezone,default_fee_cents) VALUES ('pool','Test','code','America/Denver',0)",
    )
    .run();
  for (const id of ["alice", "bob"])
    await db
      .prepare(
        "INSERT INTO memberships(pool_id,user_id,role) VALUES ('pool',?,'player')",
      )
      .run(id);
  ...
  await db
    .prepare(
      "INSERT INTO pool_weeks(pool_id,saturday,fee_cents,games_json,published) VALUES ('pool',?,0,?,1)",
    )
    .run(saturday, JSON.stringify(games));
  const app = express();
  ...
  let mailed = "";
  addRecoveryRoutes(app, db, async (_email, _purpose, token) => {
    mailed = token;
  });
  ...
  try {
    ...
    assert.equal(
      after.standings.find((r: any) => r.id === "alice").picks.length,
      2,
    );
    homePoints = 7;
    assert.equal(
      (await request(base + "/contest", "bob")).standings.find(
        (r: any) => r.id === "bob",
      ).score,
      1,
    );
    ...
    assert.equal(stale.feedUnavailable, true);
    assert.equal(stale.standings[0].score, 1);
    assert.equal(
      (
        (await db.prepare("SELECT COUNT(*) AS n FROM pick_audit").get()) as {
          n: number;
        }
      ).n,
      3,
    );
    await request("/api/auth/forgot-password", "alice", "POST", {
    ...
    await request("/api/auth/complete-email", "bob", "POST", {
      token: mailed,
      purpose: "verify",
    });
    assert(
      await db
        .prepare("SELECT 1 FROM verified_emails WHERE user_id='bob'")
        .get(),
    );
    await request(
